...
const columns = [
  ...
  { name: "PAYMENT STATUS", uid: "isComplete" },
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        ...
        console.log("Bookings from API:", data.data.bookings);
        ...
        <TableBody emptyContent={"No bookings found"} items={sortedItems}>
          {(item) => (
            <TableRow key={item.id}>
              {(columnKey) => (
                <TableCell className="text-black font-medium">{renderCell(item, columnKey)}</TableCell>
              )}
            </TableRow>
          )}
        </TableBody>
